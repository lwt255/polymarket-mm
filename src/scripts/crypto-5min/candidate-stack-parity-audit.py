...
def main():
    ...
    parser.add_argument("--fill-sec", type=int, default=28,
                        help="Collector seconds-before-end to compare bot fills against. "
                             "Default 28 reflects the pre-fill verify (1.5s delay) added in a28d47a. "
                             "Use 30 for ledgers written before that commit.")
    ...
    # With pre-fill verification deployed (commit a28d47a, 2026-04-19), the bot
    # waits 1.5s after T-33 decision and re-reads the book before firing.
    # That pushes the effective fill from T-30 to ~T-28. Use T-28 as the fill
    # target for ledgers generated by the new code; older ledgers should still
    # use T-30. --fill-sec can override.
    fill_target_sec = args.fill_sec
    ...
    for row in ledger_rows:
        ...
        if not collector:
            continue
        snaps = collector.get("snapshots") or []
        collector_fill = closest_snapshot(snaps, fill_target_sec)
