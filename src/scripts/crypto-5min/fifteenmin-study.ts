/**
 * 15-Minute BTC Market Study
 *
 * Tests: Can we apply the same directional strategy to 15-minute markets?
 *
 * 15M markets have: tighter spreads (1c vs 1.5c), 6.6x more volume ($37K vs $5.6K),
 * and 3x more time for direction to become clear.
 *
 * Tracks:
 *   - CL direction accuracy at various checkpoints (300s, 180s, 120s, 60s, 30s, 20s, 10s)
 *   - Winner ask price at each checkpoint (entry cost)
 *   - Loser bid price at each checkpoint (split strategy exit)
 *   - Book depth and spread dynamics over the candle
 *   - Comparison: does the extra time make CL more accurate earlier?
 *
 * NO ORDERS PLACED. Read-only observation.
 *
 * Run: npx tsx src/scripts/crypto-5min/fifteenmin-study.ts [numCandles]
 */
...
const OUTPUT_FILE = 'fifteenmin-study-results.json';

const CHECKPOINTS = [300, 240, 180, 120, 90, 60, 45, 30, 20, 15, 10, 5];
...
async function findCurrentMarket(): Promise<any> {
    ...
    const rounded = Math.floor(now / 900) * 900; // 15-min intervals
    for (const ts of [rounded, rounded + 900]) {
        const data = await fetchJSON(`${GAMMA}/markets?slug=btc-updown-15m-${ts}`);
...
async function monitorCandle(index: number, chainlink: ChainlinkFeed): Promise<CandleResult | null> {
        ...
        for (const cp of CHECKPOINTS) {
            if (!captured.has(cp) && secondsLeft <= cp + 0.5 && secondsLeft >= cp - 2) {
                ...
                if (upBook && downBook) {
                    ...
                    console.log(
                        `    ${String(cp).padStart(4)}s | CL: ${clDirection} ($${Math.abs(clMove).toFixed(0)}) | ` +
                        `UP: ${(upBook.bestBid * 100).toFixed(0)}/${(upBook.bestAsk * 100).toFixed(0)}c | ` +
                        `Winner ask: ${(winnerAsk * 100).toFixed(0)}c | Loser bid: ${(loserBid * 100).toFixed(0)}c (${loserBidSize.toFixed(0)} size)`
                    );
                }
            }
        }

        await new Promise(r => setTimeout(r, secondsLeft < 320 ? 500 : 3000));
    }
    ...
    const accuracy = checkpoints.map(cp => {
        ...
        // For split: if correct, you keep winner ($1) + sold loser at bid. Net = loserBid.
        // If wrong, you kept loser ($0) + sold winner at its bid. Need winner bid.
        const winnerBid = cp.clDirection === 'UP'
            ? (checkpoints.find(c => c.secondsBefore === cp.secondsBefore)?.upMid || 0.5) * 2 - (cp.winnerAsk)
            : (checkpoints.find(c => c.secondsBefore === cp.secondsBefore)?.downMid || 0.5) * 2 - (cp.winnerAsk);
        const splitPnl = clCorrect ? cp.loserBid : (winnerBid - 1);
    ...
    console.log(
        ...
        `60s: CL=${at60?.clCorrect ? 'OK' : 'X'} snipe=${at60 ? (at60.snipePnl * 100).toFixed(0) + 'c' : 'N/A'} | ` +
        `20s: CL=${at20?.clCorrect ? 'OK' : 'X'} snipe=${at20 ? (at20.snipePnl * 100).toFixed(0) + 'c' : 'N/A'}`
    );
...
function printAnalysis(results: CandleResult[]) {
    console.log('\n' + '='.repeat(80));
    console.log('15-MINUTE BTC MARKET STUDY — ANALYSIS');
...
async function main() {
    ...
    console.log(`=== 15-Minute BTC Market Study: ${NUM_CANDLES} candles ===`);
    ...
    for (let i = 0; i < remaining; i++) {
        const now = Date.now();
        const currentRound = Math.floor(now / 900000) * 900000;
        const nextCandle = currentRound + 900000;
        const intoCandle = (now - currentRound) / 1000;

        // Wait for next candle if we're too far in
        if (intoCandle > 30) {
            const waitMs = nextCandle - now + 5000;
            console.log(`  Waiting ${(waitMs / 1000).toFixed(0)}s for next 15-min candle...`);
