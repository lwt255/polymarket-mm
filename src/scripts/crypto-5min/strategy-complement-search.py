#!/usr/bin/env python3
"""
Search for complementary executable families beyond the current best stack.

Primary stack:
  - d33 cap0.75 leader interval_15m + leader_late_flip
  - d33 cap0.75 leader interval_15m + cross_0
  - d33 cap0.75 leader interval_15m + price_55_65

This script scans a broader executable rule universe and ranks candidates by:
  - out-of-sample positivity (train/test/recent)
  - incremental PnL on trades NOT already in the primary stack
  - diversification
  - optional high-win-rate bias

Usage:
  python3 src/scripts/crypto-5min/strategy-complement-search.py /path/to/pricing-data.jsonl
"""
...
def fmt(summary: dict | None) -> str:
    ...
    return (
        ...
        f"fill={summary['fill']*100:5.1f}% "
        f"edge={summary['edge_margin']*100:+5.1f}pp "
...
def candidate_rule_names(interval_tag: str, feature_tags: list[str]) -> list[str]:
    names = {"BASELINE", interval_tag, *feature_tags}
    for tag in feature_tags:
        names.add(f"{interval_tag},{tag}")
    for left, right in itertools.combinations(feature_tags, 2):
        if left.split("_", 1)[0] != right.split("_", 1)[0]:
            names.add(f"{left},{right}")
            names.add(f"{interval_tag},{left},{right}")
    return sorted(names)
...
def main() -> None:
    ...
    def flush(records: list[dict], shortlist_keys: set[tuple[str, str]] | None = None, store_primary_only: bool = False) -> None:
        nonlocal primary_ids
        if not records:
            return
        ...
        for record in records:
            ...
            ts = parse_ts(record.get("collectedAt")) or parse_ts(record.get("marketEnd"))
            ...
            for decision_sec in DECISION_SECONDS:
                ...
                for side_mode, selected_side in side_lookup.items():
                    ...
                    feature_tags = [
                        ...
                        spread_bucket(max(selected_ask - selected_bid, 0.0)),
                    ...
                    for cap in TAKER_CAPS:
                        ...
                        trade_id = f"{slug}|{record.get('marketEnd')}|{ts.isoformat() if ts else 'NA'}"

                        for rule_name in candidate_rule_names(interval_tag, feature_tags):
                            key = (base, rule_name)
                            if store_primary_only:
                                if key in PRIMARY_STACK:
                                    primary_ids.add(trade_id)
                                continue
                            if shortlist_keys is not None and key not in shortlist_keys:
                                continue
    ...
    with open(PATH, "r") as handle:
        for line in handle:
            ...
            if market_end != current_end:
                flush(batch, store_primary_only=True)
                batch = []
                current_end = market_end
            batch.append(record)
    flush(batch, store_primary_only=True)
    ...
    for key, aggregate in aggregates.items():
        ...
        if train_summary["n"] < 80 or test_summary["n"] < 25 or recent_summary["n"] < 10:
            continue
        if train_summary["avg"] <= 0 or test_summary["avg"] <= 0 or recent_summary["avg"] <= 0:
            continue
        if train_summary["edge_margin"] <= 0 or test_summary["edge_margin"] <= 0:
            continue
        if len(full_summary["cryptos"]) < 2 or max_share(full_summary["cryptos"]) > 0.70:
            continue
        shortlisted.append(
            {
                "key": key,
    ...
    shortlist_keys = {row["key"] for row in shortlisted}
    aggregates = defaultdict(
        lambda: {
            "train": empty_bucket(),
            "test": empty_bucket(),
    ...
    def flush_shortlisted(records: list[dict]) -> None:
        if not records:
            return
            ...
            for decision_sec in DECISION_SECONDS:
                ...
                for side_mode, selected_side in side_lookup.items():
                    ...
                    if t120 is not None:
                        selected_bid_t120, _, _, _ = side_fields(t120, selected_side)
                        selected_move = selected_bid - selected_bid_t120
                    depth_ratio = selected_bid_depth / selected_ask_depth if selected_ask_depth > 0 else None
                    cross_same = sum(
                        1
                        for other_crypto, prev in prev_resolutions.items()
                        if other_crypto != crypto and prev == selected_side
                    ...
                    for cap in TAKER_CAPS:
                        ...
                        for rule_name in candidate_rule_names(interval_tag, feature_tags):
                            ...
                            aggregate = aggregates[key]
                            update_bucket(aggregate["full"], crypto, interval, won, pnl, fill_ask)
                            if trade_id not in primary_ids:
                                update_bucket(aggregate["novel"], crypto, interval, won, pnl, fill_ask)
                            if ts is not None and ts < train_cut:
                                update_bucket(aggregate["train"], crypto, interval, won, pnl, fill_ask)
                            else:
                                update_bucket(aggregate["test"], crypto, interval, won, pnl, fill_ask)
    ...
    for key in shortlist_keys:
        aggregate = aggregates[key]
        full_summary = summarize(aggregate["full"])
        novel_summary = summarize(aggregate["novel"])
        train_summary = summarize(aggregate["train"])
        test_summary = summarize(aggregate["test"])
        recent_summary = summarize(aggregate["recent"])
        if not full_summary or not novel_summary or not train_summary or not test_summary or not recent_summary:
            continue
        if novel_summary["n"] < 25 or novel_summary["avg"] <= 0 or novel_summary["edge_margin"] <= 0:
            continue
        novel_share = novel_summary["n"] / full_summary["n"]
        rows.append(
            {
                "key": key,
                "full": full_summary,
    ...
    rows.sort(key=lambda row: (row["novel"]["pnl"], row["novel"]["edge_margin"], row["novel_share"]), reverse=True)
    high_wr_rows = [
        row
        for row in rows
        if row["novel"]["wr"] >= 0.65 and row["full"]["wr"] >= 0.65 and row["recent"]["wr"] >= 0.60
    ]

    print(json.dumps({"path": PATH, "count": count, "first": first_ts, "last": last_ts}, indent=2))
    primary_count = len(primary_ids)
    print(f"\nPrimary stack unique trades={primary_count}")

    print("\n=== Top Complements By Novel PnL ===")
    for row in rows[:20]:
        base, rule_name = row["key"]
        print(f"{base:24s} rule=[{rule_name}] novel_share={row['novel_share']:.1%}")
        print(f"  novel  {fmt(row['novel'])}")
        print(f"  full   {fmt(row['full'])}")
        print(f"  train  {fmt(row['train'])}")
        print(f"  test   {fmt(row['test'])}")
        print(f"  recent {fmt(row['recent'])}")

    print("\n=== Top High-Win-Rate Complements ===")
    for row in high_wr_rows[:20]:
        base, rule_name = row["key"]
        print(f"{base:24s} rule=[{rule_name}] novel_share={row['novel_share']:.1%}")
        print(f"  novel  {fmt(row['novel'])}")
        print(f"  full   {fmt(row['full'])}")
